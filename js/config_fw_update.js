...
function onGetUpdateResponse(arg) {
  ...
  if (arg.readyState == 4 && arg.status == 200) {
    ...
    if (res.HttpPushUriTargetsBusy && res.HttpPushUriTargets == "bmc_recovery") {
      document.getElementById("bmc_recovery_check").checked = true;
    } else {
      document.getElementById("bmc_recovery_check").checked = false;
    }

    ApplyTime = res.HttpPushUriOptions.HttpPushUriApplyTime.ApplyTime;
    if (ApplyTime == "OnReset") {
      document.getElementById("bmc_active_check").checked = false;
    } else {
      firmwareUpdateOptionModified = true;
      document.getElementById("bmc_active_check").checked = true;
    }
  }
}
  ...
  $.ajax({
    ...
    success : function(data, status, xhr) {
      ...
      if (xhr.readyState == 4 && xhr.status == 202) {
        var taskURI = data["@odata.id"];
        CreateCookie("TASKURI", taskURI);
        setTimeout(function () {
          getUpdateProgress();
        }, 100000);
      }
...
function getUpdateProgress() {
  var taskURI = ReadCookie("TASKURI");
