...
function readNICInfo(arg) {
  ...
  if (arg.readyState == 4 && arg.status == 200) {
    ...
    if (interfaceID.indexOf("eth") != -1) {
      ...
      network_data[global_primary_nic]["StaticNameservers"] =
          channelconfig.StaticNameServers;
      if (channelconfig.DHCPv4.DHCPEnabled)
      {
        if (channelconfig.NameServers.length > 0)
        {
          var i = 0;
          while (i < channelconfig.NameServers.length)
          {
            if (CheckIP(channelconfig.NameServers[i]))
            {
              primaryDNSServer = channelconfig.NameServers[i];
              break;
            }
            i++;
          }
          if (channelconfig.NameServers.length > 1)
          {
            for (var j = i + 1; j < channelconfig.NameServers.length; j++)
            {
              if (CheckIP(channelconfig.NameServers[j]))
              {
                secondDNSServer = channelconfig.NameServers[j];
                break;
              }
            }
          }
      ...
      {
        if (channelconfig.StaticNameServers.length > 0)
        {
          var i = 0;
          while (i < channelconfig.StaticNameServers.length)
          {
            if (CheckIP(channelconfig.StaticNameServers[i]))
            {
              primaryDNSServer = channelconfig.StaticNameServers[i];
              break;
            }
            i++;
          }
          if (channelconfig.StaticNameServers.length > 1)
          {
            for (var j = i + 1; j < channelconfig.StaticNameServers.length; j++)
            {
              if (CheckIP(channelconfig.StaticNameServers[j]))
              {
                secondDNSServer = channelconfig.StaticNameServers[j];
                break;
              }
            }
          }
...
function updateNICConfiguration(channel_id) 
{
  ...
  var DHCPenabled = network_data[channel_id].DHCPEnabled;
  var Nameservers = network_data[channel_id].Nameservers;
  var StaticNameservers = network_data[channel_id].StaticNameservers;
      ...
      if (selectedInterface.indexOf("eth2") != -1) {
        ...
        if (DHCPenabled)
        {
          if (Nameservers.length > 0)
          {
            ...
            {
              for (var j = i + 1; j < Nameservers.length; j++)
              {
                if (CheckIP(Nameservers[j]))
                {
                  itemsecondDNS = Nameservers[j];
                  break;
                }
              }
            }
        ...
        {
          if (StaticNameservers.length > 0)
          {
            var i = 0;
            while (i < StaticNameservers.length)
            {
              if (CheckIP(StaticNameservers[i]))
              {
                itemprimaryDNS = StaticNameservers[i];
                break;
              }
              i++;
            }
            if (StaticNameservers.length > 1)
            {
              for (var j = i + 1; j < StaticNameservers.length; j++)
              {
                if (CheckIP(StaticNameservers[j]))
                {
                  itemsecondDNS = StaticNameservers[j];
                  break;
                }
              }
            }
          ...
          if (StaticNameservers.length > 0)
          {
            ...
            while (i < StaticNameservers.length)
            {
              if (CheckIP(StaticNameservers[i]))
              {
                if(primaryDNS != "")
                {
                  DNSitem1.value = StaticNameservers[i];
                }
                else
                {
                  DNSitem2.value = StaticNameservers[i];
                }
                break;
              }
              i++;
            ...
            if (StaticNameservers.length > 1)
            {
              for (var j = i + 1; j < StaticNameservers.length; j++)
              {
                if (CheckIP(StaticNameservers[j]))
                {
                  DNSitem2.value = StaticNameservers[j];
                  break;
                }
              }
            }
...
function updateStaticNameServers()
{
  var ajax_url =
      "/redfish/v1/Managers/bmc/EthernetInterfaces/" + selectedInterface;
      primaryDNS = document.getElementById("_primaryDNS").value;
      secondaryDNS = document.getElementById("_secondDNS").value;
  if (secondaryDNS == "")
  {
    var ajax_param = { "StaticNameServers" : 
    [
      document.getElementById("_primaryDNS").value,
    ]
    };
  ...
  var ajax_req = new Ajax.Request(ajax_url, {
    ...
    onFailure : function(error) {
      Loading(false);
      alert("Error in saving StaticNameServers!!");
    }
  });
}
...
function saveConfiguration()
{
    ...
    onOk: function() {
            ...
            var primaryDNSServer_value = document.getElementById("_primaryDNS").value;
            var secondDNSServer_value = document.getElementById("_secondDNS").value;

            if(item.value != hostname){
                updateHostName(item.value);
            }
            if(item_static.checked && !network_data[selectedObjindex].DHCPEnabled)
            {
              if (primaryDNSServer_value != primaryDNSServer || secondDNSServer_value != secondDNSServer)
              {
                updateStaticNameServers();
              }
            }
