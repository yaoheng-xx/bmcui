...
function PageInit()
{
    ...
    document.getElementById("ButtonDeleteGroup").value = lang.LANG_CONFIG_LDAP_DELETE;
...
function LDAPGroupTableInit() {
  ...
  var LDAPGroupTable = document.getElementById("LDAPGroupTable");
...
function deleteLDAPGroup() {
  ...
  if (selected_row != "~") {
    ...
    var ajax_param = {"LDAP" : {"RemoteRoleMapping" : RemoteRoleMapping_array}};
...
function validateLDAPform() {
    ...
    if((!(/^(((cn=)|(uid=))[a-z]{1}[\w\-\.]{2,63},?)+$/i.test(LDBinDN.value.split(",")[0])) || !(/^(((cn=)|(uid=)|(ou=)|(dc=)|(o=))[a-z]{0}[\w\-\.]{1,63},?)+$/i.test(LDBinDN.value)) || LDBinDN.value.length > 63 || LDBinDN.value.length < 4)){
        error_msg_dtring += lang.LANG_CONFIG_LDAP_ERRDN + "\n";
        bool_validation = false;
    }
    if( !(/^[^ ]+$/.test(LDBinPW.value)) ){
        error_msg_dtring += lang.LANG_CONFIG_LDAP_ERRPWD + "\n";
        bool_validation = false;
    }
    if( !(/^(((cn=)|(uid=)|(ou=)|(dc=)|(o=))[a-z]{0}[\w\-\.]{1,63},?)+$/i).test(LDBase.value) 
        && !(LDBase.value.length >= 4 && LDBase.value.length <= 64)
    ){
        error_msg_dtring += lang.LANG_CONFIG_LDAP_ERRSB + "\n";
        bool_validation = false;
    }
    if (LDGroupName.value == '' && selected_row != '') {
      error_msg_dtring += lang.LANG_CONFIG_LDAP_ERRGROUP_NAME + "\n";
      bool_validation = false;
    }
    if (LDGroupPrivilege.value == '' && selected_row != '') {
      error_msg_dtring += lang.LANG_CONFIG_LDAP_ERRGROUP_PRIVILEGE + "\n";
      bool_validation = false;
    }
    if(!bool_validation){
        alert(lang.LANG_CONFIG_LDAP_ERRTITLE + "\n" + error_msg_dtring, {type: "pre"});
        error_msg_dtring = "";
    }else{
        error_msg_dtring = "";
        saveLDAPconfig();
