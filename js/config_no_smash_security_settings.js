...
function PageInit() {
    ...
    passwordService.options["Low"].text = lang.CONF_COMPLEX_PASSWORD_LOW;
    passwordService.options["Medium"].text = lang.CONF_COMPLEX_PASSWORD_MEDIUM;
    passwordService.options["High"].text = lang.CONF_COMPLEX_PASSWORD_HIGH;
...
function btnSaved() {
  ...
  if (enableHostinterface.checked != hostInterfaceEnabledPrevious) {
    change_count += 1;
    updateHostInterfaceEnabled(enableHostinterface.checked);
  }
...
function HandleDelayedSuccessStatus(originalRequest) {
  change_count = change_count - 1;
  if (change_count == 0) {
    Loading(false);
    alert(lang.CONF_SUCCESS_MSG + " Please be noted that settings will take effect in 30 seconds", {
      title : lang.LANG_GENERAL_SUCCESS,
      onClose : function() {
        location.href = "/page/config_no_smash_security_settings.html";
      }
...
function updateHostInterfaceEnabled(value) {
  var ajax_url = "/redfish/v1/Managers/bmc";
  var obj = { "Oem": { "HostInterface": { "Enabled": value } } };
  var ajax_req = new Ajax.Request(ajax_url, {
    method : 'PATCH',
    contentType : "application/json",
    parameters : JSON.stringify(obj),
    onSuccess : HandleDelayedSuccessStatus,
    onFailure : function() {
      Loading(false);
      alert(lang.CONF_USER_LOCK_ERROR_MSG);
    }
...
function readNICConfig(arg) {
  ...
  if (arg.readyState == 4 && arg.status == 200) {
    ...
    for (i = 0; i < channelInfo["Members@odata.count"]; i++) {
      ...
      {
          hostInterfaceEnabledPrevious = hostInterfaceEnabled;
          hostInterfaceEnabled = false;
      }
    }
