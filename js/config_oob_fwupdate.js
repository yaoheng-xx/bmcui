...
function getUpdateStatusResponse(args) {
  ...
  if (args.readyState == 4 && args.status == 200) {
    ...
    if (cnt != 0) {
      if (ReadCookie("BIOSTASKURI") != null) {
        taskURI = ReadCookie("BIOSTASKURI");
    ...
    } else if (cnt == 0 && ReadCookie("BIOSTASKURI") != null) {
      EraseCookie("BIOSTASKURI");
      EraseCookie("BIOSSTARTTIME");
    }
  }
}
...
function checkStartTimeResp(args) {
  ...
  if (args.readyState == 4 && args.status == 200) {
    var res = JSON.parse(args.responseText);
    if (ReadCookie("BIOSSTARTTIME") != null) {
      var st = ReadCookie("BIOSSTARTTIME");
      if (res.StartTime == st) {
        LoadFileStart();
        getUpdateProgress();
      } else {
...
function setBIOSRegionOptionRecovery() {
  ...
  var ajax_req = new Ajax.Request(ajax_url, {
    ...
    onFailure : function() {
      ...
      document.getElementById("uploadBtn").disabled = false;
      document.getElementById("Bios_backup_check").disabled = false;
      document.getElementById("Bios_backup_check").checked = false;
...
function setBIOSRegionOptionReset() {
  ...
  document.getElementById("FileBrowse").disabled = true;
  document.getElementById("uploadBtn").disabled = true;
  ApplyTime = document.getElementById("Bios_nvram_check").checked == false
                      ? 'OnReset'
                      : 'Immediate';
  var ajax_url = '/redfish/v1/UpdateService';
  var data = {
...
function onGetUpdateResponse(arg) {
  ...
  if (arg.readyState == 4 && arg.status == 200) {
    ...
    if (res.HttpPushUriTargetsBusy && res.HttpPushUriTargets == "bios_recovery") {
      document.getElementById("Bios_backup_check").checked = true;
    } else {
      document.getElementById("Bios_backup_check").checked = false;
    }
  ...
  $.ajax({
    ...
    success : function(data, status, xhr) {
      if (capFileFlag) {
         ...
         $("#fwready-value").text(lang.LANG_CONFIG_OOB_SAVED)
           .addClass('fwupd-active');
         $("#fwready-value").show();
      } else {
        if (xhr.readyState == 4 && xhr.status == 202) {
          taskURI = data["@odata.id"];
          CreateCookie("BIOSTASKURI", taskURI);
          setStartTimeCookie();
          setTimeout(function () {
            getUpdateProgress();
          }, 30000);
...
function setStartTimeCookieResp(args) {
  ...
  if (args.readyState == 4 && args.status == 200) {
    var res = JSON.parse(args.responseText);
    CreateCookie("BIOSSTARTTIME", res.StartTime);
  }
}
