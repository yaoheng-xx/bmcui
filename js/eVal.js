...
var eVal =
{
	...
	trimdash : function(stringToTrim)
	{
...
//       // Now just compare directly to some invalid IPs
	...
	ipv6 : function (ipstr, allowlocal, allow00)
	{
		...
		function checkHexa(str, allowlocal, allow00)
		{
			...
			for (i=0; i<str.length; i++) {
				...
				if(!allowlocal) {
					if (i==0 && ((decip == 0xFE80) || ((decip & 0xFEC0) == 0xFEC0))) {
						//alert ("FE80->Link local and Site Local IP's are Not allowed");
						return true;
					}
				}
				if(!allow00) {
	...
	/*
	 * This function is used to validate various username like IPMI, AD, so on.
	 * @param username string, value of username to validate.
	 * @param fltr string, specify the username filter like IPMI, AD, etc.,
	 * @param minlen number, minimum length for username.
	 * @param maxlen number, maximum length for username.
	 * @return boolean, true-validation success, false-validation fails.
	 */
	username : function (username, fltr, minlen, maxlen)
	{
		...
		if (fltr == "IPMI") {	//Filter for IPMI user validation
			invalidchar_filter = /[,$;|/:!()~?%*+<=>{}#[\]^.`"\'\\&\s]/;
		} else if (fltr == "AD") {	//Filter for AD user validation
			invalidchar_filter = /[.,;:|=+*?<>@"/\\[\] ]/;
		} else {		//Filter for SMTP user validation
                        invalidchar_filter = /[,$;|/:!()~?%*+<=>{}#[\]^@`"\'\\&\s]/;
		}

		if (username.match(invalidchar_filter)) {
			return false;
		}
	...
	ldap : function (str, property)
	{
		...
		for (i = min-1; i < v.length; i++) {
                        if ((!dc.test(v[i])) && (!ou.test(v[i])) && (!o.test(v[i])) && (!(cn.test(v[i]) || uid.test(v[i])))) {
                                return false;
			}
		}
	...
	*/
	...
	/*
	 * This function is used to validate the string value against the specified
	 * set of invalid file path special characters.
	 * @param : str -> Required. String to be validated.
	 * @return : true -> if 'str' is not having matched invalid file path
	 * special characters.
	 * @return : false -> if 'str' is having matched invalid file path 
	 * special characters.
	 */
	filePath : function(str) {
		var filter = "<>*.?:|";	//Used to hold the Invalid file path characters
		return eVal.specialCharacter(str, filter);
	},
