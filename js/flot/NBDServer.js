...
function NBDServer(endpoint, file, id, token) {
  this.file = file;
  this.id = id;
  ...
  this.sendbytes = 0;
  this._eventHandlers = {
    'ready': function () { alert("No onready callback function!"); },
    'mount': function () { alert("No onmount callback function!"); },
    'unmount': function () { alert("No onunmount callback function!"); },
    'error': function () { alert("No onerror callback function!"); },
    'process': function() { }
  };
  ...
  this.getState = function() {
    var storageState = sessionStorage.getItem("VM_State");
    if (this.state != storageState) {
      console.log("VM_State different");
    }
    return this.state;
  }

  this._on_ws_error = function(ev) {
    msg = 'error code: ' + ev.code + ' reason: ' + ev.reason;
    console.log('vm/0/' + id + msg);
    this._eventHandlers.error(msg);
  };

  this._on_ws_close = function(ev) {
    var msg = ' closed with code: ' + ev.code;
    if (ev.reason.length > 0) {
      msg = msg + ' reason: ' + ev.reason;
      this._eventHandlers.error(msg);
      return;
    }
    console.log('vm/0/' + id + msg);
    this._eventHandlers.unmount();
  };
  ...
  this._on_ws_message = function(ev) {
    ...
    for (;;) {
      ...
      if (consumed < 0) {
        console.log('handler[state=' + this.state + '] returned error ' +
                    consumed);
        this.stop();
        break;
      }
  ...
  this._handle_option = function(buf) {
    if (buf.byteLength < 16) {
      console.log('invalid option length');
      return 0;
    }
    ...
    if (buf.byteLength < 16 + len) {
      console.log('invalid option payload length');
      return 0;
    }

    switch (opt) {
    case NBD_OPT_EXPORT_NAME:
      console.log('handle option NBD_OPT_EXPORT_NAME');
      var n = 10;
  ...
  this._handle_cmd_read = function(req) {
    ...
    this.sendbytes = this.sendbytes + req.length;
    this._eventHandlers.process(this.sendbytes);
