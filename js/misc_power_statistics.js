      ...
      if (content.hasOwnProperty("Domains")) {
        ...
        for (i = 0; i < PStatic_Data.length; i++) {
          if (PStatic_Data[i].hasOwnProperty("Power")) {
            ...
            var subSystemTitle;
            if (PStatic_Data[i].Id.search('memory') == 0) {
              subSystemTitle = lang.LANG_MISC_POWER_MEMORY_SUBSYSTEM;
            } else if (PStatic_Data[i].Id.search('cpu') == 0) {
              subSystemTitle = lang.LANG_MISC_POWER_CPU_SUBSYSTEM;
            } else if (PStatic_Data[i].Id.search('entirePlatform') == 0) {
              subSystemTitle = lang.LANG_MISC_POWER_ENTIRE_PLATFORM;
            }
            RowData.push([
              id, subSystemTitle,
...
function secondsToString(seconds){
    ...
    if(numdays != 0)
        return numdays + lang.LANG_MISC_POWER_PERIOD_DAYS +
                numhours + lang.LANG_MISC_POWER_PERIOD_HOURS +
                numminutes + lang.LANG_MISC_POWER_PERIOD_MINUTES +
                numseconds + lang.LANG_MISC_POWER_PERIOD_SECONDS;
    else if(numhours != 0)
        return numhours + lang.LANG_MISC_POWER_PERIOD_HOURS +
                numminutes + lang.LANG_MISC_POWER_PERIOD_MINUTES +
                numseconds + lang.LANG_MISC_POWER_PERIOD_SECONDS;
    else if(numminutes != 0)
        return numminutes + lang.LANG_MISC_POWER_PERIOD_MINUTES +
                numseconds + lang.LANG_MISC_POWER_PERIOD_SECONDS;
    else
        return numseconds + lang.LANG_MISC_POWER_PERIOD_SECONDS;
}
...
function showComponentPwr(originalRequest)
{
    ...
    if (originalRequest.readyState == 4 && originalRequest.status == 200) {
      ...
      if (gCount == 0) {
        ...
        RowData3.push([ totalWattId + 1, lang.LANG_MISC_POWER_TOTAL_INPUT, totalWatt ]);
