...
function pageInit() {
  ...
  //Only support from auto reflesh from 30s to never. See HSD 2103655403. 
  if (browser_ie) {
...
function showSensorCategory(sensorRecNum, selectSensorType) {
  ...
  for (var i = 0; i < sensorRecNum; i++) {
    ...
    if (j < sensorCategoryObj.length)
      continue;
    var option = new Option(sensorType, sensorType);
  ...
  for (j = 1; j < sensorCategoryObj.options.length; j++) {
    if (sensorCategoryObj.options[j].value == "Temperatures") {
      sensorCategoryObj.options[j].text = lang.LANG_SENSOR_SNRTYPE1;
    } else if (sensorCategoryObj.options[j].value == "Fan") {
      sensorCategoryObj.options[j].text = lang.LANG_SENSOR_SNRTYPE4;
    } else if (sensorCategoryObj.options[j].value == "Voltages") {
      sensorCategoryObj.options[j].text = lang.LANG_SENSOR_SNRTYPE2;
    } else if (sensorCategoryObj.options[j].value == "Current") {
      sensorCategoryObj.options[j].text = lang.LANG_SENSOR_SNRTYPE3;
    } else if (sensorCategoryObj.options[j].value == "Power") {
      sensorCategoryObj.options[j].text = lang.LANG_SENSOR_SNRTYPE2D;
    } else if (sensorCategoryObj.options[j].value == "utilization") {
      sensorCategoryObj.options[j].text = lang.LANG_SENSOR_SNRTYPE_UTILIZATION;
    } else if (sensorCategoryObj.options[j].value == "AirFlow") {
      sensorCategoryObj.options[j].text = lang.LANG_SENSOR_SNRTYPE_AIRFLOW;
    }
  }
}
