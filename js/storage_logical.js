...
function getLogicalId(val) {
  return lang.LANG_SYS_STORAGE_STORAGE + val.replace("_", "") +
          ", " + lang.LANG_SYS_STORAGE_LOGIC_DEVICES;
}
...
function checkProperty(obj, key) {
  if (obj.hasOwnProperty(key)) {
    if (key == 'CapacityBytes' || key == 'StripSizeBytes') {
      ...
      return tmp;
    } else if (key == 'ReadCachePolicy') {
      if (obj[key] == 'Off') {
        return 'NoReadAhead';
      }
    } else if (key == 'WriteCachePolicy') {
      if (obj[key] == 'UnprotectedWriteBack') {
        return 'AlwaysWriteBack';
      } else if (obj[key] == 'ProtectedWriteBack') {
        return 'WriteBack';
      }
    }
    return obj[key];
