...
function ReplyCPUInfo(arg){
    ...
    if (arg.readyState == 4 && arg.status == 200) {
      ...
      var cpu_present = org_req.Status.State;
      if(cpu_present.includes("Absent"))
        return;
      var cpu_socket = org_req.Socket;
      var cpu_num = Number(cpu_socket.match(/\d+/g));
      document.getElementById("CPU_InfoBox_" + cpu_num).innerHTML =
          "<div align=\"left\" id=\"CPU_" + cpu_num + "\">" +
          ...
          "</label></td>" +
          "<td><span id=\"procFamily_" + cpu_num + "\"" +
          " class=\"labeltext\"></span>" +
          "</td></tr>" +
          // Speed -> yes
          "<tr><td align=\"right\" class=\"bold\">" +
          ...
          "</label></td>" +
          "<td><span id=\"socketType_" + cpu_num + "\"" +
          " class=\"labeltext\"></span>" +
          "</td></tr>" +
          // Status - yes
          "<tr><td align=\"right\" class=\"bold\">" +
          ...
          "</label></td>" +
          "<td><span id=\"assetTag_" + cpu_num + "\"" +
          " class=\"labeltext\"></span>" +
          "</td></tr>" +
          "</table>" +
          "</fieldset>" +
          "</div>";
      document.getElementById("socketNo_" + cpu_num).textContent =
          org_req.Socket;
      document.getElementById("manuf_" + cpu_num).textContent =
          org_req.Manufacturer;
      document.getElementById("procFamily_" + cpu_num).textContent =
      ...
      document.getElementById("speed_" + cpu_num).textContent =
          dis_ghz + "GHz";
      document.getElementById("cores_" + cpu_num).textContent =
          org_req.TotalCores;
      document.getElementById("version_" + cpu_num).textContent =
          org_req.Version;
      if (org_req.hasOwnProperty("Oem")) {
        document.getElementById("voltage_" + cpu_num).textContent =
            checkProperty(org_req.Oem, 'Voltage');
      }
      document.getElementById("status_" + cpu_num).textContent =
          org_req.Status.State;
      document.getElementById("socketType_" + cpu_num).textContent =
            checkProperty(org_req.Oem, 'ProcessorUpgrade');
      document.getElementById("procType_" + cpu_num).textContent =
          org_req.ProcessorType;
      if (org_req.hasOwnProperty("Oem")) {
        document.getElementById("assetTag_" + cpu_num).textContent =
      ...
      if (org_req.hasOwnProperty("Oem")) {
        ...
        document.getElementById("procSig_" + cpu_num).textContent = final_str;
      }
      cpu_count++;
    }
}
